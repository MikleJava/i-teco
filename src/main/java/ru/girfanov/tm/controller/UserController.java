...
public class UserController {
    ...
    @Autowired
    private AdapterUtil<UserDto, User> adapter;
    ...
    public String registration(@ModelAttribute("user") UserDto userDto, BindingResult bindingResult) {
        ...
        userService.persist(adapter.convertToEntity(userDto));
    ...
    public String authorization(@RequestParam("login") @NotNull final String login, final ModelMap modelMap) {
        @Nullable final UserDto userDto = adapter.convertToEntityDto(userService.findOneByLogin(login));
        if(userDto == null) {
    ...
    public String logout(final ModelMap modelMap, final Principal principal) {
        ...
        @Nullable final UserDto userDto = adapter.convertToEntityDto(userService.findOne(userId));
        if(userDto == null) {
            modelMap.addAttribute("error", "User does not exist");
            return "error";
        }
